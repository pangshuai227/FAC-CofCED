...
os.environ.setdefault("CUDA_VISIBLE_DEVICES", os.environ.get("COFCED_CUDA_VISIBLE_DEVICES", "0"))
...
DEFAULT_DATA_ROOT = pjoin(CODE_ROOT, "dataset", "LIAR-RAW")
ROOT_PROJ_PATH = os.environ.get("COFCED_DATA_ROOT", DEFAULT_DATA_ROOT)
...
LOG_DIR = os.environ.get("COFCED_LOG_DIR", pjoin(CODE_ROOT, "dataset", "logs"))
...
def train_model(n_epochs=n_epochs,
    ...
    start_epoch = 0
    if resume_model_url:
        log.logger.info("Resuming from checkpoint: %s", resume_model_url)
        resumed_model = torch.load(resume_model_url, map_location=device)
        if isinstance(resumed_model, nn.Module):
            model = resumed_model.to(device)
        else:
            model.load_state_dict(resumed_model)
        matched_epoch = re.search(r"_epoch(\d+)_", os.path.basename(resume_model_url))
        if matched_epoch:
            start_epoch = int(matched_epoch.group(1)) + 1
        log.logger.info("Resume start epoch index: %d (display epoch %d)", start_epoch, start_epoch + 1)
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        if SAVE_EACH_EPOCH:
            name = ROOT_PROJ_PATH.split('/')[-1]
            last_model_url = from_project_root("%s_last_epoch%d.pt" % (name, epoch))
            torch.save(model, last_model_url)
            log.logger.info("saved last epoch checkpoint: %s", last_model_url)
        # print(f'Acc:{train_acc / i:.6f}, prec:{train_precision / i:.6f}, recall:{train_recall / i:.6f}, f1:{train_f1 / i:.6f}')
...
if __name__ == '__main__':
    ...
    train_model(resume_model_url=os.environ.get("COFCED_RESUME_MODEL"))
